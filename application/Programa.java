...
public class Programa {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            MostrarMenu(); // Mostra opções do Menu principal o usuário no console
            ...
            switch (opcao) {
                ...
                case "2" -> ImprimirProntuario(); // Imprime prontuario e pergunta sobre atendimento
                // ( Se atender ele sai da fila, se não ele continua na fila como pendente)

                case "3" -> VerPontruarios(); // Visualiza prontuários pendentes/atendidos/cancelados ( Pendentes por
    ...
    // Exibe o menu principal com as opções disponíveis no sistema.

    private static void MostrarMenu() {
        System.out.println("\n╔═══════════════════════════════════════════╗");
        System.out.println("║         SISTEMA DE FILA HOSPITALAR        ║");
        System.out.println("╠═══════════════════════════════════════════╣");
        System.out.println("║ 1 - Adicionar prontuario médica           ║");
     ...
     * permitido.
     *
     * @parametro min Valor mínimo permitido para a opção.
     * 
     * @parametro max Valor máximo permitido para a opção.
     * 
     * @returna Opção válida digitada pelo usuário.
     * 
     */
    ...
    /**
     * Solicita informações do prontuario ao usuário e insere na fila de atendimento.
     */
    private static void AddProntuario() {
        ...
        String sintoma = lerStringNaoVazia();
    ...
    /**
     * Imprime a próxima prontuario da fila e pergunta se o prontuario será atendido.
    ...
    private static void ImprimirProntuario() {
        ...
        while (!fila.estaVazia()) {
            ...
            if (opcao.equals("s")) {
                resp = ValidarConfirmacao();
                if (resp.equalsIgnoreCase("s")) {
                    p = fila.remover();
                    p.setStatus(Status.ATENDIDO);
                    System.out.println("\n╔══════════════════════════════════════╗");
                    System.out.println("║    prontuario atendido com sucesso.    ║");
                    System.out.println("╚══════════════════════════════════════╝");
                    System.out.println(p.toProntuario());
                } else {
                ...
                break; // termina após manipular um prontuario
            } else if (opcao.equals("p")) {
                resp = ValidarConfirmacao();
                // Envia o prontuario ao final da fila mantendo prioridade
                Prontuario pulado = fila.remover();
                fila.inserir(pulado);
                System.out.println("\n╔══════════════════════════════════════╗");
    ...
    /**
     * Permite ao usuário escolher entre visualizar prontuarios pendentes (por risco),
     * atendidos (ordem alfabética) ou cancelados.
     */
            ...
            switch (opcao) {
                ...
                case "3" -> {
                    ...
                    for (Cancelamento c : cancel) {
                        System.out.println(c); // já imprime formatado com prontuario e justificativa
                    }
                }
    ...
    private static void cancelarProntuario() {
        ...
        Optional<Prontuario> prontuarioEncontrado = lista.stream()
                .filter(p -> p.getNome().toLowerCase().equals(nomeBusca))
                .findFirst();
        ...
        String justificativa = lerStringNaoVazia();

        // Usa método centralizado de cancelamento
    ...
    /**
     * Lê uma string do usuário que não pode ser vazia.
     *
    ...
    /**
     * Lê um inteiro positivo válido do usuário.
     *
    ...
    /**
     * Lê uma resposta do usuário que deve ser 's' ou 'n'.
     *
     * @return "s" ou "n" validado.
     */
    ...
    public static void AddProntuarioCancelado(HeapProntuario fila) {
        ...
        Prontuario prontuarioExemplo = new Prontuario("Carlos Souza", 52, "Falta de ar", "vermelho");

        // Inserir na fila (opcional, apenas para simular o fluxo real)
        fila.inserir(prontuarioExemplo);

        // Cancelar usando o método centralizado
        Cancelamento.cancelarProntuario(prontuarioExemplo, "prontuario solicitou atendimento particular", fila);
    }
